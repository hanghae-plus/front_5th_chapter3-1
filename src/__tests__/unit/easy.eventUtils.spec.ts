...
describe('getFilteredEvents', () => {
  ...
  it("검색어 '이벤트 2'에 맞는 이벤트만 반환한다", () => {
    const filtered = getFilteredEvents(testEvents, '이벤트 2', baseDate, 'month');
  ...
  it('주간 뷰에서 2025-07-01 주의 이벤트만 반환한다', () => {
    ...
    const filtered = getFilteredEvents(testEvents, '', currentDate, 'week');
  ...
  it('월간 뷰에서 2025년 7월의 모든 이벤트를 반환한다', () => {
    const filtered = getFilteredEvents(testEvents, '', baseDate, 'month');
  ...
  it("검색어 '이벤트'와 주간 뷰 필터링을 동시에 적용한다", () => {
    const currentDate = new Date('2025-07-01');
    const filtered = getFilteredEvents(testEvents, '이벤트', currentDate, 'week');
  ...
  it('검색어가 대소문자를 구분하지 않고 작동한다', () => {
    const filtered1 = getFilteredEvents(testEvents, '이벤트', baseDate, 'month');
    const filtered2 = getFilteredEvents(testEvents, '이벤트', baseDate, 'month');
  ...
  it('월의 경계에 있는 이벤트를 올바르게 필터링한다', () => {
    ...
    const filtered = getFilteredEvents(testEvents, '', borderDate, 'week');
  ...
  it('위치 정보로 이벤트를 검색한다', () => {
    const filtered = getFilteredEvents(testEvents, '회의실', baseDate, 'month');
