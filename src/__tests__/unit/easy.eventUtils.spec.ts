...
describe('getFilteredEvents', () => {
  ...
  // '주간 뷰에서 검색어 '팀 회의'에 맞는 이벤트만 반환한다'와 동일한 케이스라고 생각해서 주석처리 했습니다.
  // it("검색어 '이벤트'와 주간 뷰 필터링을 동시에 적용한다", () => {});

  // getFilteredEvents는 currentDate를 인자로 받기에 모든 이벤트를 반환하기는 어렵기 때문에 '특정 월의 이벤트를 반환한다'는 조건을 추가하려고 했습니다.
  // 하지만 이럴 경우 결국 위의 ‘월간 뷰에서 2025년 7월의 모든 이벤트를 반환한다’와 동일한 케이스가 된다고 생각해서 주석처리 했습니다.
