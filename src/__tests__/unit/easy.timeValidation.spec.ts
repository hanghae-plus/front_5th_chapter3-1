...
describe('getTimeErrorMessage >', () => {
  it('시작 시간이 종료 시간보다 늦을 때 에러 메시지를 반환한다', () => {
    const { startTimeError, endTimeError } = getTimeErrorMessage('12:00', '10:00');
  ...
  it('시작 시간이 종료 시간보다 빠를 때 null을 반환한다', () => {
    const { startTimeError, endTimeError } = getTimeErrorMessage('09:00', '10:00');
    expect(startTimeError).toBeNull();
    expect(endTimeError).toBeNull();
  });

  it('시작 시간이 비어있을 때 null을 반환한다', () => {
    const { startTimeError, endTimeError } = getTimeErrorMessage('', '10:00');
    expect(startTimeError).toBeNull();
    expect(endTimeError).toBeNull();
  });
