...
describe('getWeekDates', () => {
  it('주중의 날짜(수요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    expect(getWeekDates(new Date('2025-05-07'))).toEqual([
      new Date('2025-05-04'),
      new Date('2025-05-05'),
      new Date('2025-05-06'),
      new Date('2025-05-07'),
  ...
  it('주의 시작(월요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    expect(getWeekDates(new Date('2025-05-04'))).toEqual([
      new Date('2025-05-04'),
      new Date('2025-05-05'),
      new Date('2025-05-06'),
      new Date('2025-05-07'),
  ...
  it('주의 끝(일요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    expect(getWeekDates(new Date('2025-05-11'))).toEqual([
      new Date('2025-05-11'),
      new Date('2025-05-12'),
      new Date('2025-05-13'),
      new Date('2025-05-14'),
  ...
  it('윤년의 2월 29일을 포함한 주를 올바르게 처리한다', () => {
    const year = 2024;
    expect(isLeapYear(year)).toBe(true);
    expect(getWeekDates(new Date(`${year}-02-29`))).toEqual([
      new Date(`${year}-02-25`),
      new Date(`${year}-02-26`),
      new Date(`${year}-02-27`),
      new Date(`${year}-02-28`),
...
describe('getEventsForDay', () => {
  ...
  it('특정 날짜(1일)에 해당하는 이벤트만 정확히 반환한다', () => {
    expect(getEventsForDay(events, 1)).toEqual([firstDayEvent])
  });

  it('해당 날짜에 이벤트가 없을 경우 빈 배열을 반환한다', () => {
    expect(getEventsForDay(events, 5)).toEqual([])
  });
...
describe('formatWeek', () => {
  ...
  it('월의 마지막 주에 대해 올바른 주 정보를 반환한다', () => {
    expect(formatWeek(new Date('2025-05-30'))).toBe('2025년 5월 5주')
  });
  ...
  it('윤년 2월의 마지막 주에 대해 올바른 주 정보를 반환한다', () => {
    const year = 2024;
    expect(isLeapYear(year)).toBe(true);
    expect(formatWeek(new Date(`${year}-02-28`))).toBe('2024년 2월 5주')
  });

  it('평년 2월의 마지막 주에 대해 올바른 주 정보를 반환한다', () => {
    const year = 2025;
    expect(isLeapYear(year)).toBe(false);
    expect(formatWeek(new Date(`${year}-02-29`))).toBe('2025년 2월 4주')
  });
});
...
describe('fillZero', () => {
  ...
  test('value가 지정된 size보다 큰 자릿수를 가지면 원래 값을 그대로 반환한다', () => {
    expect(fillZero(5135,2)).toBe('5135')
  });
});

describe('formatDate', () => {
  it('날짜를 YYYY-MM-DD 형식으로 포맷팅한다', () => {
    expect(formatDate(new Date('2025-12-11'))).toBe('2025-12-11')
  });

  it('day 파라미터가 제공되면 해당 일자로 포맷팅한다', () => {
    expect(formatDate(new Date('2025-12-11'), 22)).toBe('2025-12-22')
  });
