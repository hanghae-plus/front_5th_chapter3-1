...
import { SAMPLE_EVENTS } from '../SampleEvent';
...
describe('getEventsForDay', () => {
  it('특정 날짜(1일)에 해당하는 이벤트만 정확히 반환한다', () => {
    ...
    expect(getEventsForDay(events, 1)).toEqual([day1]);
  });

  it('해당 날짜에 이벤트가 없을 경우 빈 배열을 반환한다', () => {
    expect(getEventsForDay(SAMPLE_EVENTS, 1)).toEqual([]);
  });

  it('날짜가 0일 경우 빈 배열을 반환한다', () => {
    expect(getEventsForDay(SAMPLE_EVENTS, 0)).toEqual([]);
  });

  it('날짜가 32일 이상인 경우 빈 배열을 반환한다', () => {
    expect(getEventsForDay(SAMPLE_EVENTS, 32)).toEqual([]);
  });
});
...
describe('formatDate', () => {
  ...
  it('일이 한 자리 수일 때 앞에 0을 붙여 포맷팅한다', () => {
    ...
    expect(formatDate(date, 1)).toBe('2025-10-01');
  });
});
