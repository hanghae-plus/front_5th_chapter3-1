...
describe('getDaysInMonth', () => {
  ...
  it('윤년(2020년)의 2월에 대해 29일을 반환한다', () => {
    expect(getDaysInMonth(2020, 2)).toBe(29);
  });

  it('평년(2019년)의 2월에 대해 28일을 반환한다', () => {
...
describe('getWeekDates', () => {
  ...
  it('주의 끝(일요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const weekDates = getWeekDates(new Date('2025-05-11'));
    expect(weekDates).toEqual([
      new Date('2025-05-11'),
      new Date('2025-05-12'),
      new Date('2025-05-13'),
  ...
  it('연도를 넘어가는 주의 날짜를 정확히 처리한다 (연초)', () => {
    const weekDates = getWeekDates(new Date('2026-01-01'));
    expect(weekDates).toEqual([
      new Date('2025-12-28'),
      new Date('2025-12-29'),
      new Date('2025-12-30'),
  ...
  it('윤년의 2월 29일을 포함한 주를 올바르게 처리한다', () => {
    const weekDates = getWeekDates(new Date('2020-02-29'));
    expect(weekDates).toEqual([
      new Date('2020-02-23'),
      new Date('2020-02-24'),
      new Date('2020-02-25'),
  ...
  it('월의 마지막 날짜를 포함한 주를 올바르게 처리한다', () => {
    const weekDates = getWeekDates(new Date('2025-04-29'));
    expect(weekDates).toEqual([
      new Date('2025-04-27'),
      new Date('2025-04-28'),
      new Date('2025-04-29'),
...
describe('getEventsForDay', () => {
  it('특정 날짜(1일)에 해당하는 이벤트만 정확히 반환한다', () => {
    ...
    expect(getEventsForDay(events, date)).toEqual([
      {
        id: '1',
        title: '팀 회의',
        date: '2025-05-20',
...
describe('formatWeek', () => {
  ...
  it('윤년 2월의 마지막 주에 대해 올바른 주 정보를 반환한다', () => {
    ...
    const result = formatWeek(date);
    expect(result).toBe('2020년 2월 4주');
  });

  it('평년 2월의 마지막 주에 대해 올바른 주 정보를 반환한다', () => {
    const date = new Date(2021, 1, 28);
    const result = formatWeek(date);
    expect(result).toBe('2021년 3월 1주');
  });
});
...
describe('formatDate', () => {
  ...
  it('day 파라미터가 제공되면 해당 일자로 포맷팅한다', () => {
    const date = new Date(2025, 4, 14);
    const formattedDate = formatDate(date, 20);
  ...
  it('월이 한 자리 수일 때 앞에 0을 붙여 포맷팅한다', () => {
    ...
    const formattedDate = formatDate(date);
    expect(formattedDate).toBe('2025-05-05');
  });
