...
describe('getWeekDates', () => {
  it('주중의 날짜(수요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    ...
    expect(formatDate(weekDates[0])).toBe('2025-05-11');
    expect(formatDate(weekDates[6])).toBe('2025-05-17');
  });
  ...
  it('연도를 넘어가는 주의 날짜를 정확히 처리한다 (연말)', () => {
    ...
    const weekDates = getWeekDates(date);
    expect(weekDates).toHaveLength(7);
    expect(formatDate(weekDates[0])).toBe('2025-12-28');
    expect(formatDate(weekDates[6])).toBe('2026-01-03');
  });
  ...
  it('윤년의 2월 29일을 포함한 주를 올바르게 처리한다', () => {
    ...
    const weekDates = getWeekDates(date);
    expect(weekDates).toHaveLength(7);
    expect(formatDate(weekDates[0])).toBe('2024-02-25');
    expect(formatDate(weekDates[6])).toBe('2024-03-02');
  });
  ...
  it('월의 마지막 날짜를 포함한 주를 올바르게 처리한다', () => {
    ...
    const weekDates = getWeekDates(date);
    expect(weekDates).toHaveLength(7);
    expect(formatDate(weekDates[0])).toBe('2025-05-25');
    expect(formatDate(weekDates[6])).toBe('2025-05-31');
  });
});
...
describe('isDateInRange', () => {
  ...
  it('시작일이 종료일보다 늦은 경우 모든 날짜에 대해 false를 반환한다', () => {
    ...
    expect(isDateInRange(invalidStart, invalidStart, invalidEnd)).toBe(false);
    expect(isDateInRange(invalidEnd, invalidStart, invalidEnd)).toBe(false);
  });
});
...
describe('formatDate', () => {
  ...
  it('일이 한 자리 수일 때 앞에 0을 붙여 포맷팅한다', () => {
    ...
    expect(formatDate(date)).toBe('2025-10-05');
  });
});
