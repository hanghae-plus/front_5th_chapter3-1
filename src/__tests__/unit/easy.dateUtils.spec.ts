...
describe('getDaysInMonth', () => {
  it('주어진 월과 연도를 기반으로 해당 월의 일수를 반환해야 한다', () => {
    expect(getDaysInMonth(2025, 1)).toBe(31);
    expect(getDaysInMonth(2025, 4)).toBe(30);
    expect(getDaysInMonth(2025, 6)).toBe(30);
  });

  it('주어진 월이 2월이면서 윤년인 경우 29일을 반환해야 한다', () => {
    expect(getDaysInMonth(2024, 2)).toBe(29);
  });

  it('주어진 월이 2월이면서 평년인 경우 28일을 반환해야 한다', () => {
    expect(getDaysInMonth(2023, 2)).toBe(28);
  });

  it('주어진 월이 유효하지 않은 경우 31일을 반환해야 한다', () => {
    expect(getDaysInMonth(2025, 13)).toBe(31);
  });
});

// !! describe 수정 및 테스트 추가하기
describe('getWeekDates', () => {
  it('주중의 날짜(수요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const dates = getWeekDates(new Date('2025-07-09'));
  ...
  it('주의 시작(월요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const dates = getWeekDates(new Date('2025-07-07'));

    expect(dates).toHaveLength(7);
  });

  it('주의 끝(일요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const dates = getWeekDates(new Date('2025-07-13'));

    expect(dates).toHaveLength(7);
  });

  it('연도를 넘어가는 주의 날짜를 정확히 처리한다 (연말)', () => {
    const dates = getWeekDates(new Date('2024-12-31'));
  ...
  it('월의 마지막 날짜를 포함한 주를 올바르게 처리한다', () => {
    const dates = getWeekDates(new Date('2025-07-31'));
