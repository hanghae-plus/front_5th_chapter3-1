...
describe('getWeekDates', () => {
  it('주중의 날짜(수요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    ...
    const weekDates = getWeekDates(date).map((d) => d.toISOString().split('T')[0]);
...
describe('getEventsForDay', () => {
  ...
  it('날짜가 32일 이상인 경우 빈 배열을 반환한다', () => {
    const result = getEventsForDay(mockEvents, 31);
    expect(result).toEqual([]);
  });
});
...
describe('formatWeek', () => {
  it('월의 중간 날짜에 대해 올바른 주 정보를 반환한다', () => {
    const result = formatWeek(new Date('2025-07-17'));
...
describe('isDateInRange', () => {
  ...
  it('시작일이 종료일보다 늦은 경우 모든 날짜에 대해 false를 반환한다', () => {
    const result = isDateInRange(new Date('2025-08-01'), new Date('2025-08-01'), rangeEnd);
    expect(result).toBe(false);
  });
});
