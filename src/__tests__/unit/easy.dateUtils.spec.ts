...
describe('getEventsForDay', () => {
  it('특정 날짜(21일)에 해당하는 이벤트만 정확히 반환한다', () => {
    expect(getEventsForDay(MOCK_DATA, 21)).toEqual([
      {
  ...
  it('해당 날짜에 이벤트가 없을 경우 빈 배열을 반환한다', () => {
    expect(getEventsForDay(MOCK_DATA, 11)).toEqual([]);
  });

  it('날짜가 0일 경우 빈 배열을 반환한다', () => {
    expect(getEventsForDay(MOCK_DATA, 0)).toEqual([]);
  });

  it('날짜가 32일 이상인 경우 빈 배열을 반환한다', () => {
    expect(getEventsForDay(MOCK_DATA, 32)).toEqual([]);
  });
});
...
describe('formatDate', () => {
  ...
  it('일이 한 자리 수일 때 앞에 0을 붙여 포맷팅한다', () => {
    const date = new Date('2025-05-01');
    expect(formatDate(date)).toBe('2025-05-01');
  });
});
