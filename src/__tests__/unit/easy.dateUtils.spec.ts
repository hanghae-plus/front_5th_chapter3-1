...
describe('getWeekDates', () => {
  it('주중의 날짜(수요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const date = new Date('2025-05-14'); // 수요일
    const expectedDates = [
      new Date('2025-05-11'),
      new Date('2025-05-12'),
      new Date('2025-05-13'),
  ...
  it('주의 시작(월요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const date = new Date('2025-05-12'); // 월요일
    const expectedDates = [
      new Date('2025-05-11'),
      new Date('2025-05-12'),
      new Date('2025-05-13'),
...
describe('getWeeksAtMonth', () => {
  it('2025년 7월 1일의 올바른 주 정보를 반환해야 한다', () => {
    ...
    expect(getWeeksAtMonth(date)).toEqual(expectedWeeks);
  });
});
...
describe('formatWeek', () => {
  it('월의 중간 날짜에 대해 올바른 주 정보를 반환한다', () => {
    ...
    const expectedFormat = '2025년 5월 3주';
    expect(formatWeek(date)).toBe(expectedFormat);
  });

  it('월의 첫 주에 대해 올바른 주 정보를 반환한다', () => {
    const date = new Date('2025-05-01');
    const expectedFormat = '2025년 5월 1주';
    expect(formatWeek(date)).toBe(expectedFormat);
  });

  it('월의 마지막 주에 대해 올바른 주 정보를 반환한다', () => {
    const date = new Date('2025-05-31');
    const expectedFormat = '2025년 5월 5주';
    expect(formatWeek(date)).toBe(expectedFormat);
  });

  it('연도가 바뀌는 주에 대해 올바른 주 정보를 반환한다', () => {
    const date = new Date('2024-12-31');
    const expectedFormat = '2025년 1월 1주';
    expect(formatWeek(date)).toBe(expectedFormat);
  });

  it('윤년 2월의 마지막 주에 대해 올바른 주 정보를 반환한다', () => {
    const date = new Date('2024-02-29');
    const expectedFormat = '2024년 2월 5주';
    expect(formatWeek(date)).toBe(expectedFormat);
  });

  it('평년 2월의 마지막 주에 대해 올바른 주 정보를 반환한다', () => {
    const date = new Date('2025-02-28');
    const expectedFormat = '2025년 2월 4주';
    expect(formatWeek(date)).toBe(expectedFormat);
  });
});
...
describe('formatMonth', () => {
  it("2025년 7월 10일을 '2025년 7월'로 반환한다", () => {
    ...
    expect(formatMonth(date)).toBe(expectedFormat);
  });
});
...
describe('formatDate', () => {
  ...
  it('일이 한 자리 수일 때 앞에 0을 붙여 포맷팅한다', () => {
    ...
    const formattedDate = formatDate(date, 5);
    expect(formattedDate).toBe('2025-07-05');
  });
});
