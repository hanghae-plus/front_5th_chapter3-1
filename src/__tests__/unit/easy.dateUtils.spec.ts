...
describe('getWeekDates', () => {
  ...
  it('윤년의 2월 29일을 포함한 주를 올바르게 처리한다', () => {
    const weekDates = getWeekDates(new Date('2000-02-29'));
    assertDate(weekDates[0], new Date('2000-02-27'));
    assertDate(weekDates[1], new Date('2000-02-28'));
    assertDate(weekDates[2], new Date('2000-02-29'));
    assertDate(weekDates[3], new Date('2000-03-01'));
    assertDate(weekDates[4], new Date('2000-03-02'));
    assertDate(weekDates[5], new Date('2000-03-03'));
    assertDate(weekDates[6], new Date('2000-03-04'));
  });

  it('월의 마지막 날짜를 포함한 주를 올바르게 처리한다', () => {
    const weekDates = getWeekDates(new Date('2025-05-31'));
    assertDate(weekDates[0], new Date('2025-05-25'));
    assertDate(weekDates[1], new Date('2025-05-26'));
    assertDate(weekDates[2], new Date('2025-05-27'));
    assertDate(weekDates[3], new Date('2025-05-28'));
    assertDate(weekDates[4], new Date('2025-05-29'));
    assertDate(weekDates[5], new Date('2025-05-30'));
    assertDate(weekDates[6], new Date('2025-05-31'));
  });
});
...
describe('getEventsForDay', () => {
  it('특정 날짜(1일)에 해당하는 이벤트만 정확히 반환한다', () => {
    expect(getEventsForDay(MOCK_EVENTS, 1)).toHaveLength(2);
  });

  it('해당 날짜에 이벤트가 없을 경우 빈 배열을 반환한다', () => {
    expect(getEventsForDay(MOCK_EVENTS, 31)).toHaveLength(0);
  });

  it('날짜가 0일 경우 빈 배열을 반환한다', () => {
    expect(getEventsForDay(MOCK_EVENTS, 0)).toHaveLength(0);
  });

  it('날짜가 32일 이상인 경우 빈 배열을 반환한다', () => {
    expect(getEventsForDay(MOCK_EVENTS, 32)).toHaveLength(0);
  });
});
...
describe('formatWeek', () => {
  ...
  it('윤년 2월의 마지막 주에 대해 올바른 주 정보를 반환한다', () => {
    expect(formatWeek(new Date('2000-02-29'))).toBe('2000년 3월 1주');
  });
...
describe('isDateInRange', () => {
  ...
  it('범위의 시작일 2025-07-01에 대해 true를 반환한다', () => {
    expect(isDateInRange(new Date('2025-07-01'), rangeStart, rangeEnd)).toBeTruthy();
  });

  it('범위의 종료일 2025-07-31에 대해 true를 반환한다', () => {
    expect(isDateInRange(new Date('2025-07-31'), rangeStart, rangeEnd)).toBeTruthy();
  });
  ...
  it('시작일이 종료일보다 늦은 경우 모든 날짜에 대해 false를 반환한다', () => {
    expect(isDateInRange(new Date(), new Date('2025-05-14'), new Date('2025-05-01'))).toBeFalsy();
  });
});
