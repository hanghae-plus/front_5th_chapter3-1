...
describe('getWeeksAtMonth', () => {
  it('2025년 7월 1일의 올바른 주 정보를 반환해야 한다', () => {
    ...
    expect(weeksAtMonth[0]).toEqual([null, null, 1, 2, 3, 4, 5]);
    expect(weeksAtMonth[1]).toEqual([6, 7, 8, 9, 10, 11, 12]);
    expect(weeksAtMonth[2]).toEqual([13, 14, 15, 16, 17, 18, 19]);
    expect(weeksAtMonth[3]).toEqual([20, 21, 22, 23, 24, 25, 26]);
    expect(weeksAtMonth[4]).toEqual([27, 28, 29, 30, 31, null, null]);
  });
});

describe('getEventsForDay', () => {
  it('특정 날짜(20일)에 해당하는 이벤트만 정확히 반환한다', () => {
    const events = getEventsForDay(realEvents.events as Event[], 20);
  ...
  it('날짜가 32일 이상인 경우 빈 배열을 반환한다', () => {
    const events = getEventsForDay(realEvents.events as Event[], 33);
    expect(events).toHaveLength(0);
  });
});
...
describe('formatWeek', () => {
  it('월의 중간 날짜에 대해 올바른 주 정보를 반환한다', () => {
    const formattedWeek = formatWeek(new Date('2025-05-14'));
    expect(formattedWeek).toBe('2025년 5월 3주');
  });

  it('월의 첫 주에 대해 올바른 주 정보를 반환한다', () => {
    const formattedWeek = formatWeek(new Date('2025-05-01'));
    expect(formattedWeek).toBe('2025년 5월 1주');
  });

  it('월의 마지막 주에 대해 올바른 주 정보를 반환한다', () => {
    const formattedWeek = formatWeek(new Date('2025-05-31'));
    expect(formattedWeek).toBe('2025년 5월 5주');
  });

  it('연도가 바뀌는 주에 대해 올바른 주 정보를 반환한다', () => {
    const formattedWeek = formatWeek(new Date('2024-12-31'));
    expect(formattedWeek).toBe('2025년 1월 1주');
  });

  it('윤년 2월의 마지막 주에 대해 올바른 주 정보를 반환한다', () => {
    const formattedWeek = formatWeek(new Date('2024-2-29'));
    expect(formattedWeek).toBe('2024년 2월 5주');
  });

  it('평년 2월의 마지막 주에 대해 올바른 주 정보를 반환한다', () => {
    const formattedWeek = formatWeek(new Date('2025-2-28'));
...
describe('fillZero', () => {
  test("5를 2자리로 변환하면 '05'를 반환한다", () => {
    const filledNumber = fillZero(5, 2);
    expect(filledNumber).toBe('05');
  });

  test("10을 2자리로 변환하면 '10'을 반환한다", () => {
    const filledNumber = fillZero(10, 2);
    expect(filledNumber).toBe('10');
  });

  test("3을 3자리로 변환하면 '003'을 반환한다", () => {
    const filledNumber = fillZero(3, 3);
    expect(filledNumber).toBe('003');
  });

  test("100을 2자리로 변환하면 '100'을 반환한다", () => {
    const filledNumber = fillZero(100, 2);
    expect(filledNumber).toBe('100');
  });

  test("0을 2자리로 변환하면 '00'을 반환한다", () => {
    const filledNumber = fillZero(0, 2);
    expect(filledNumber).toBe('00');
  });

  test("1을 5자리로 변환하면 '00001'을 반환한다", () => {
    const filledNumber = fillZero(1, 5);
    expect(filledNumber).toBe('00001');
  });

  test("소수점이 있는 3.14를 5자리로 변환하면 '03.14'를 반환한다", () => {
    const filledNumber = fillZero(3.14, 5);
    expect(filledNumber).toBe('03.14');
  });

  test('size 파라미터를 생략하면 기본값 2를 사용한다', () => {
    const filledNumber = fillZero(1);
    expect(filledNumber).toBe('01');
  });

  test('value가 지정된 size보다 큰 자릿수를 가지면 원래 값을 그대로 반환한다', () => {
    const filledNumber = fillZero(10000, 2);
    expect(filledNumber).toBe('10000');
  });
});
