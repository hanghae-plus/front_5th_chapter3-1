...
describe('getWeekDates', () => {
  it('주중의 날짜(수요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const result = getWeekDates(new Date('2025-05-07'));
    expect(result[3].getDay()).toBe(3);
  });

  it('주의 시작(월요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const result = getWeekDates(new Date('2025-05-05'));
  ...
  it('연도를 넘어가는 주의 날짜를 정확히 처리한다 (연말)', () => {
    const result = getWeekDates(new Date('2025-12-28'));
    expect(result[0].getDay()).toBe(0);
  });

  it('연도를 넘어가는 주의 날짜를 정확히 처리한다 (연초)', () => {
    const result = getWeekDates(new Date('2026-01-02'));
    expect(result[5].getDay()).toBe(5);
  });
  ...
  it('윤년의 2월 29일을 포함한 주를 올바르게 처리한다', () => {
    ...
    expect(result[0].toDateString()).toBe(new Date('2024-02-25').toDateString());
    expect(result[6].toDateString()).toBe(new Date('2024-03-02').toDateString());
    const hasFeb29 = result.some((date) => date.getDate() === 29 && date.getMonth() === 1);
...
describe('getWeeksAtMonth', () => {
  it('2025년 7월 1일의 올바른 주 정보를 반환해야 한다', () => {
    ...
    expect(result[0]).toEqual([null, null, 1, 2, 3, 4, 5]);
  });
});
...
describe('formatWeek', () => {
  it('월의 중간 날짜에 대해 올바른 주 정보를 반환한다', () => {
    const result = formatWeek(new Date('2025-05-30'));
    expect(result).toBe('2025년 5월 5주');
  });

  it('월의 첫 주에 대해 올바른 주 정보를 반환한다', () => {
    const result = formatWeek(new Date('2025-06-01'));
...
describe('isDateInRange', () => {
  ...
  it('시작일이 종료일보다 늦은 경우 모든 날짜에 대해 false를 반환한다', () => {
    const result = isDateInRange(new Date('2025-08-01'), rangeEnd, rangeStart);
    expect(result).toBe(false);
  });
});
...
describe('formatDate', () => {
  ...
  it('일이 한 자리 수일 때 앞에 0을 붙여 포맷팅한다', () => {
    const result = formatDate(new Date('2025-10-1'), 1);
