...
describe('getWeekDates', () => {
  it('주중의 날짜(수요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    ...
    expect(weekDates.map((date) => date.toISOString().slice(0, 10))).toEqual([
      '2025-05-11',
      '2025-05-12',
      '2025-05-13',
      '2025-05-14',
  ...
  it('주의 시작(월요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const weekDates = getWeekDates(new Date('2025-05-05'));
...
describe('getEventsForDay', () => {
  ...
  it('특정 날짜(1일)에 해당하는 이벤트만 정확히 반환한다', () => {
    const theEvent = getEventsForDay(events, 20);
    expect(theEvent).toHaveLength(1);
    expect(theEvent[0].title).toBe('팀 회의');
  });

  it('해당 날짜에 이벤트가 없을 경우 빈 배열을 반환한다', () => {
    const theEvent = getEventsForDay(events, 19);
    expect(theEvent).toHaveLength(0);
  });

  it('날짜가 0일 경우 빈 배열을 반환한다', () => {
    const theEvent = getEventsForDay(events, 0);
    expect(theEvent).toHaveLength(0);
  });

  it('날짜가 32일 이상인 경우 빈 배열을 반환한다', () => {
    const theEvent = getEventsForDay(events, 32);
    expect(theEvent).toHaveLength(0);
  });
});
...
describe('formatDate', () => {
  ...
  it('day 파라미터가 제공되면 해당 일자로 포맷팅한다', () => {
    const date = new Date('2025-05-12');
    expect(formatDate(date, 14)).toBe('2025-05-14');
  });

  it('월이 한 자리 수일 때 앞에 0을 붙여 포맷팅한다', () => {
    const date = new Date('2025-06-01');
    expect(formatDate(date)).toBe('2025-06-01');
  });
