...
describe('getDaysInMonth', () => {
  ...
  it('유효하지 않은 월에 대해 예외를 발생시킨다', () => {
    expect(() => getDaysInMonth(2025, 13)).toThrow('월은 1부터 12 사이의 값이어야 합니다.');
  });
});

describe('getWeekDates', () => {
  it('주중의 날짜(수요일)에 대해 해당 주의 일요일부터 토요일의 날짜들을 반환한다.', () => {
    const result = getWeekDates(new Date('2025-07-09'));
  ...
  it('주의 시작(일요일)에 대해 해당 주의 일요일부터 토요일의 날짜들을 반환한다.', () => {
  ...
  it('주의 끝(토요일)에 대해 해당 주의 일요일부터 토요일의 날짜들을 반환한다.', () => {
    const result = getWeekDates(new Date('2025-07-12'));
  ...
  it('월의 마지막 날짜를 포함한 주를 올바르게 처리한다', () => {
    const result = getWeekDates(new Date('2025-07-31'));
...
describe('getEventsForDay', () => {
  ...
  it('해당 날짜에 이벤트가 없을 경우 빈 배열을 반환한다', () => {
    const result = getEventsForDay(MOCK_EVENTS, 3);
    expect(result).toEqual([]);
  });
  ...
  it('날짜가 32일 이상인 경우 빈 배열을 반환한다', () => {
    const result = getEventsForDay(MOCK_EVENTS, 32);
    expect(result).toEqual([]);
  });
});
