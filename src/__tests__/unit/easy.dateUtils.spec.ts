...
describe('getWeekDates', () => {
  ...
  it('주의 끝(일요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const dates = getWeekDates(new Date('2025-05-11'));
    expect(dates).toEqual([
      new Date('2025-05-11'),
      new Date('2025-05-12'),
      new Date('2025-05-13'),
  ...
  it('연도를 넘어가는 주의 날짜를 정확히 처리한다 (연초)', () => {
    const dates = getWeekDates(new Date('2026-01-01'));
    expect(dates).toEqual([
      new Date('2025-12-28'),
      new Date('2025-12-29'),
      new Date('2025-12-30'),
...
describe('formatWeek', () => {
  it('월의 중간 날짜에 대해 올바른 주 정보를 반환한다', () => {
    const result = formatWeek(new Date('2025-05-12'));
    expect(result).toBe('2025년 5월 3주');
  });

  it('월의 첫 주에 대해 올바른 주 정보를 반환한다', () => {
    const result = formatWeek(new Date('2025-05-01'));
...
describe('isDateInRange', () => {
  ...
  it('시작일이 종료일보다 늦은 경우 모든 날짜에 대해 false를 반환한다', () => {
    const badStart = new Date('2025-08-01');
    const badEnd = new Date('2025-07-01');
    const datesToTest = [
      new Date('2025-06-30'),
      new Date('2025-07-01'),
      new Date('2025-07-15'),
    ...
    datesToTest.forEach((date) => {
      expect(isDateInRange(date, badStart, badEnd)).toBe(false);
    });
  });
});
...
describe('fillZero', () => {
  ...
  test('value가 지정된 size보다 큰 자릿수를 가지면 원래 값을 그대로 반환한다', () => {
    expect(fillZero(999, 2)).toBe('999');
...
describe('formatDate', () => {
  it('날짜를 YYYY-MM-DD 형식으로 포맷팅한다', () => {
    ...
    expect(formatDate(date)).toBe('2025-05-12');
  });
