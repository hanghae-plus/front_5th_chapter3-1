...
describe('getWeekDates', () => {
  it('주중의 날짜(수요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const dates = getWeekDates(new Date('2025-05-14'));
  ...
  it('주의 시작(월요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const dates = getWeekDates(new Date('2025-05-12'));
  ...
  it('주의 끝(일요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const dates = getWeekDates(new Date('2025-05-18'));
  ...
  it('연도를 넘어가는 주의 날짜를 정확히 처리한다 (연말)', () => {
    const dates = getWeekDates(new Date('2025-12-31'));
  ...
  it('연도를 넘어가는 주의 날짜를 정확히 처리한다 (연초)', () => {
    const dates = getWeekDates(new Date('2025-01-01'));
  ...
  it('월의 마지막 날짜를 포함한 주를 올바르게 처리한다', () => {
    const dates = getWeekDates(new Date('2025-05-31'));
...
describe('getEventsForDay', () => {
  it('특정 날짜(1일)에 해당하는 이벤트만 정확히 반환한다', () => {
    const events = getEventsForDay(MOCK_EVENTS, 1);

    expect(events).toEqual([MOCK_EVENTS[0]]);
  });

  it('해당 날짜에 이벤트가 없을 경우 빈 배열을 반환한다', () => {
    const events = getEventsForDay(MOCK_EVENTS, 2);

    expect(events).toEqual([]);
  });

  it('날짜가 0일 경우 빈 배열을 반환한다', () => {
    const events = getEventsForDay(MOCK_EVENTS, 0);

    expect(events).toEqual([]);
  });

  it('날짜가 32일 이상인 경우 빈 배열을 반환한다', () => {
    const events = getEventsForDay(MOCK_EVENTS, 32);
...
describe('formatWeek', () => {
  it('월의 중간 날짜에 대해 올바른 주 정보를 반환한다', () => {
    const week = formatWeek(new Date('2025-05-15'));
  ...
  it('연도가 바뀌는 주에 대해 올바른 주 정보를 반환한다', () => {
    const week = formatWeek(new Date('2025-12-31'));
...
describe('fillZero', () => {
  test("5를 2자리로 변환하면 '05'를 반환한다", () => {
    const text = fillZero(5);

    expect(text).toBe('05');
  });

  test("10을 2자리로 변환하면 '10'을 반환한다", () => {
    const text = fillZero(10);

    expect(text).toBe('10');
  });

  test("3을 3자리로 변환하면 '003'을 반환한다", () => {
    const text = fillZero(3, 3);

    expect(text).toBe('003');
  });

  test("100을 2자리로 변환하면 '100'을 반환한다", () => {
    const text = fillZero(100, 2);

    expect(text).toBe('100');
  });

  test("0을 2자리로 변환하면 '00'을 반환한다", () => {
    const text = fillZero(0, 2);

    expect(text).toBe('00');
  });

  test("1을 5자리로 변환하면 '00001'을 반환한다", () => {
    const text = fillZero(1, 5);

    expect(text).toBe('00001');
  });

  test("소수점이 있는 3.14를 5자리로 변환하면 '03.14'를 반환한다", () => {
    const text = fillZero(3.14, 5);

    expect(text).toBe('03.14');
  });

  test('size 파라미터를 생략하면 기본값 2를 사용한다', () => {
    const text = fillZero(1);

    expect(text).toBe('01');
  });

  test('value가 지정된 size보다 큰 자릿수를 가지면 원래 값을 그대로 반환한다', () => {
    const text = fillZero(12345, 2);
