...
describe('getWeekDates', () => {
  it('주중의 날짜(수요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    ...
    expect(result[4].getDate()).toBe(3);
    expect(result[5].getDate()).toBe(4);
  ...
  it('연도를 넘어가는 주의 날짜를 정확히 처리한다 (연말)', () => {
    ...
    expect(result[6].getDate()).toBe(3);
    expect(result[6].getMonth()).toBe(0);
    expect(result[6].getFullYear()).toBe(2026);
  });
  ...
  it('윤년의 2월 29일을 포함한 주를 올바르게 처리한다', () => {
    ...
    expect(result.length).toBe(7);
    expect(result[0].getDate()).toBe(25);
    expect(result[4].getDate()).toBe(29);
    expect(result[6].getDate()).toBe(2);
  });
  ...
  it('월의 마지막 날짜를 포함한 주를 올바르게 처리한다', () => {
    ...
    expect(result[5].getDate()).toBe(1);
    expect(result[6].getDate()).toBe(2);
  });
});
...
describe('getWeeksAtMonth', () => {
  it('2025년 7월 1일의 올바른 주 정보를 반환해야 한다', () => {
    ...
    expect(result[0][6]).toBe(5);
    expect(result[1][0]).toBe(6);
    expect(result[4][0]).toBe(27);
    expect(result[4][4]).toBe(31);
    expect(result[4][5]).toBe(null);
    expect(result[4][6]).toBe(null);
  });
});
...
describe('isDateInRange', () => {
  ...
  it('시작일이 종료일보다 늦은 경우 모든 날짜에 대해 false를 반환한다', () => {
    ...
    expect(isDateInRange(new Date('2025-07-15'), invalidStart, invalidEnd)).toBe(false);
  });
});
