...
it('검색어가 제목, 설명, 위치 중 하나라도 일치하면 해당 이벤트를 반환해야 한다', () => {
  ...
  act(() => {
    result.current.setSearchTerm('주간 팀');
  });
  expect(result.current.filteredEvents).toEqual([
    {
      id: '2b7545a6-ebee-426c-b906-2329bc8d62bd',
...
it('현재 뷰(주간/월간)에 해당하는 이벤트만 반환해야 한다', () => {
  const { result: monthResult } = renderHook(() =>
    useSearch(events, new Date('2025-05-01'), 'month')
  );
  const { result: weekResult } = renderHook(() =>
    useSearch(events, new Date('2025-05-01'), 'week')
  );
  act(() => {
    monthResult.current.setSearchTerm('팀');
  });
  ...
  act(() => {
    weekResult.current.setSearchTerm('팀');
  });
