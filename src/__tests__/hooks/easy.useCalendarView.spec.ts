...
describe('초기 상태', () => {
  beforeAll(() => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date('2025-10-01')); // 현재 시간을 2024-10-01로 고정
  });
...
describe('기능 확인', () => {
  ...
  it("view를 'week'으로 변경하면 내부 상태가 'week'로 업데이트된다.", () => {
    const { result } = renderHook(() => useCalendarView());
  ...
  it("currentDate가 '2025-01-01' 변경되면 holidays는 1월 휴일인 신정, 설날이 지정되어 있어야 한다", async () => {
    const { result } = renderHook(() => useCalendarView());
