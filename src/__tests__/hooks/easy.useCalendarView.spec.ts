...
describe('초기 상태', () => {
  ...
  it('currentDate는 오늘 날짜인 "2025-10-01"이어야 한다', () => {
    ...
    assertDate(result.current.currentDate, new Date(Date.UTC(2025, 9, 1)));
  });
...
it("주간 뷰에서 다음으로 navigate시 7일 후 '2025-10-08' 날짜로 지정이 된다", () => {
  ...
  assertDate(result.current.currentDate, new Date(Date.UTC(2025, 9, 8)));
});
...
it("주간 뷰에서 이전으로 navigate시 7일 후 '2025-09-24' 날짜로 지정이 된다", () => {
  ...
  assertDate(result.current.currentDate, new Date(Date.UTC(2025, 8, 24)));
});
...
it("월간 뷰에서 다음으로 navigate시 한 달 전 '2025-11-01' 날짜여야 한다", () => {
  ...
  assertDate(result.current.currentDate, new Date(Date.UTC(2025, 10, 1)));
});
...
it("월간 뷰에서 이전으로 navigate시 한 달 전 '2025-09-01' 날짜여야 한다", () => {
  ...
  assertDate(result.current.currentDate, new Date(Date.UTC(2025, 8, 1)));
});
