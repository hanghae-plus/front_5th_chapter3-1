...
describe('초기 상태', () => {
  ...
  it('오늘 날짜가 10월이라면 holidays는 10월 휴일들이 지정되어 있어야 한다', () => {
    const { result } = renderHook(() => useCalendarView());
    act(() => {
...
it("오늘 날짜가 10월 1일이라면 주간 뷰에서 다음으로 navigate시 7일 후 '2025-10-08' 날짜로 지정이 된다", () => {
  const { result } = renderHook(() => useCalendarView());
...
it("오늘 날짜가 10월 1일이라면 주간 뷰에서 이전으로 navigate시 7일 후 '2025-09-24' 날짜로 지정이 된다", () => {
  const { result } = renderHook(() => useCalendarView());
...
it("오늘 날짜가 10월 1일이라면 월간 뷰에서 다음으로 navigate시 한 달 전 '2025-11-01' 날짜여야 한다", () => {
  const { result } = renderHook(() => useCalendarView());
...
it("오늘 날짜가 10월 1일이라면 월간 뷰에서 이전으로 navigate시 한 달 전 '2025-09-01' 날짜여야 한다", () => {
  const { result } = renderHook(() => useCalendarView());
...
it("currentDate가 '2025-01-01' 변경되면 holidays 는 1월 휴일날짜로 업데이트되어야 한다", async () => {
  const { result } = renderHook(() => useCalendarView());
