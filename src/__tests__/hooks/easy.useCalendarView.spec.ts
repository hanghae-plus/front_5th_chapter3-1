...
describe('useCalendarView', () => {
  ...
  it(`currentDate는 오늘 날짜인 ${TODAY}이어야 한다`, () => {
    const { result } = renderHook(() => useCalendarView());
    assertDate(result.current.currentDate, TODAY_DATE);
  });
  ...
  it("주간 뷰에서 다음으로 navigate 시 7일 후 '2025-10-08' 날짜로 지정된다", () => {
    const { result } = renderHook(() => useCalendarView());
    act(() => {
      result.current.setView('week');
    });
  ...
  it("주간 뷰에서 이전으로 navigate 시 7일 전 '2025-09-24' 날짜로 지정된다", () => {
    const { result } = renderHook(() => useCalendarView());
    act(() => {
      result.current.setView('week');
    });
  ...
  it("월간 뷰에서 다음으로 navigate 시 한 달 후 '2025-11-01' 날짜여야 한다", () => {
    const { result } = renderHook(() => useCalendarView());
    act(() => {
  ...
  it("월간 뷰에서 이전으로 navigate 시 한 달 전 '2025-09-01' 날짜여야 한다", () => {
    const { result } = renderHook(() => useCalendarView());
    act(() => {
      result.current.setView('month');
      result.current.navigate('prev');
