...
describe('초기 상태', () => {
  ...
  it('초기 currentDate는 테스트 상 지정 날짜인 "2025-10-01"이어야 한다', () => {
    const { result } = renderHook(() => useCalendarView());

    assertDate(result.current.currentDate, new Date('2025-10-01'));
  });

  it('초기 holidays는 10월 휴일인 추석, 개천절, 한글날이 지정되어 있어야 한다', () => {
    const { result } = renderHook(() => useCalendarView());
...
describe('view 변경', () => {
  it("view를 'week'으로 변경 시 view가 'week'로 변경된다", () => {
    const { result } = renderHook(() => useCalendarView());
  ...
  it("currentDate가 '2025-03-03' 변경되면 3월 휴일 '삼일절'로 업데이트되어야 한다", async () => {
    ...
    act(() => {
      result.current.setCurrentDate(new Date('2025-03-03'));
    });
