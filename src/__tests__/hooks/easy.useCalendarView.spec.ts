...
it("주간 뷰에서 다음으로 navigate시 7일 후 '2025-10-08' 날짜로 지정이 된다", () => {
  ...
  expect(result.current.currentDate.toISOString().split('T')[0]).toBe('2025-10-08');
});
...
it("주간 뷰에서 이전으로 navigate시 7일 후 '2025-09-24' 날짜로 지정이 된다", () => {
  ...
  expect(result.current.currentDate.toISOString().split('T')[0]).toBe('2025-09-24');
});
...
it("월간 뷰에서 다음으로 navigate시 한 달 전 '2025-11-01' 날짜여야 한다", () => {
  ...
  expect(result.current.currentDate.toISOString().split('T')[0]).toBe('2025-11-01');
});
...
it("월간 뷰에서 이전으로 navigate시 한 달 전 '2025-09-01' 날짜여야 한다", () => {
  ...
  expect(result.current.currentDate.toISOString().split('T')[0]).toBe('2025-09-01');
});
