...
describe('초기 상태', () => {
  ...
  it('holidays는 10월 휴일인 개천절, 한글날이 지정되어 있어야 한다', () => {
    ...
    expect(result.current.holidays).toHaveProperty('2025-10-03');
    expect(result.current.holidays).toHaveProperty('2025-10-09');
  });
});
...
it("currentDate가 '2025-01-01' 변경되면 1월 휴일 '신정'으로 업데이트되어야 한다", async () => {
  ...
  expect(result.current.holidays).toHaveProperty('2025-01-01');
});
