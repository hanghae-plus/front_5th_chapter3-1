...
describe('초기 상태', () => {
  const mockToday = new Date('2025-10-01T00:00:00');
  ...
  it('currentDate는 오늘 날짜인 "2025-10-01"이어야 한다', () => {
    ...
    assertDate(currentDate, mockToday);
...
it("주간 뷰에서 다음으로 navigate 시 7일 후 '2025-10-08' 날짜로 지정이 된다", () => {
  const mockToday = new Date('2025-10-01T00:00:00');
  ...
  const expectedDate = new Date('2025-10-08T00:00:00');
...
it("주간 뷰에서 이전으로 navigate시 7일 후 '2025-09-24' 날짜로 지정이 된다", () => {
  ...
  const expectedDate = new Date('2025-09-24T00:00:00');
  assertDate(result.current.currentDate, expectedDate);
...
it("월간 뷰에서 다음으로 navigate시 한 달 전 '2025-11-01' 날짜여야 한다", () => {
  ...
  const expectedDate = new Date('2025-11-01T00:00:00');
  assertDate(result.current.currentDate, expectedDate);
...
it("currentDate가 '2025-01-01' 변경되면 1월 휴일 '신정'으로 업데이트되어야 한다", async () => {
  const mockToday = new Date('2025-01-01T00:00:00');
  vi.setSystemTime(mockToday);

  const { result } = renderHook(() => useCalendarView());

  const expectedDate = new Date('2025-01-01T00:00:00');
  assertDate(result.current.currentDate, expectedDate);
});
