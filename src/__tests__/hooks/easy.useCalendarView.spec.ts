...
it("주간 뷰에서 다음으로 navigate시 7일 후 '2025-10-08' 날짜로 지정이 된다", () => {
  ...
  assertDate(result.current.currentDate, nextWeek);
});
...
it("주간 뷰에서 이전으로 navigate시 7일 후 '2025-09-24' 날짜로 지정이 된다", () => {
  ...
  const prevWeek = new Date('2025-09-24');
  assertDate(result.current.currentDate, prevWeek);
});
...
it("월간 뷰에서 다음으로 navigate시 한 달 전 '2025-11-01' 날짜여야 한다", () => {
  ...
  assertDate(result.current.currentDate, nextMonth);
});
...
it("월간 뷰에서 이전으로 navigate시 한 달 전 '2025-09-01' 날짜여야 한다", () => {
  ...
  const nextMonth = new Date('2025-09-01');
  assertDate(result.current.currentDate, nextMonth);
});
