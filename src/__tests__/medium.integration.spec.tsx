...
const triggerScheduleEvent = async (
  ...
  const formFields = [
    ...
    { key: 'date', label: '날짜', value: date },
    { key: 'startTime', label: '시작 시간', value: startTime },
    { key: 'endTime', label: '종료 시간', value: endTime },
    { key: 'description', label: '설명', value: description },
    { key: 'location', label: '위치', value: location },
  ];
...
describe('검색 기능', () => {
  ...
  it("'팀 회의'를 검색하면 해당 제목을 가진 일정이 리스트에 노출된다", async () => {
    vi.setSystemTime(new Date('2025-05-21T00:00:00Z'));
    const initialEventsWithTeamMeeting: Event[] = [mockEvents.teamMeeting, mockEvents.otherMeeting];
