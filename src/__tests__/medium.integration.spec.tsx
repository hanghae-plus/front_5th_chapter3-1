...
beforeEach(() => {
  vi.setSystemTime(new Date('2025-05-20T09:50:00Z'));
...
describe('일정 CRUD 및 기본 기능', () => {
  ...
  it('일정을 삭제하고 더 이상 조회되지 않는지 확인한다', async () => {
    ...
    await waitFor(() => expect(within(eventList).queryByText(eventTitle)).not.toBeInTheDocument());
  });
});
...
it('notificationTime을 1로 하면 지정 시간 1분 전 알람 텍스트가 노출된다', async () => {
  vi.setSystemTime(new Date('2025-05-20T00:59:00Z'));
