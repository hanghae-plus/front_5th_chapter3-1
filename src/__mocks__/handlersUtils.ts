...
export const setupMockHandlerCreation = (initEvents = [] as Event[]) => {
  ...
  server.use(
    ...
    http.post('/api/events', async ({ request }) => {
      const targetEvent = (await request.json()) as Event;
      mockEvents.push(targetEvent);
...
export const setupMockHandlerUpdating = (initEvents = [] as Event[]) => {
  ...
  server.use(
    ...
    http.put('/api/events/:id', async ({ params, request }) => {
      ...
      const targetIndex = mockEvents.findIndex((event) => event.id === id);
      mockEvents[targetIndex] = { ...mockEvents[targetIndex], ...updatedEvent };
      return HttpResponse.json(mockEvents[targetIndex]);
    })
  );
};
...
export const setupMockHandlerDeletion = (initEvents = [] as Event[]) => {
  ...
  server.use(
    ...
    http.delete('/api/events/:id', async ({ params }) => {
      const { id } = params;
      const targetIndex = mockEvents.findIndex((event) => event.id === id);
      mockEvents.splice(targetIndex, 1);
