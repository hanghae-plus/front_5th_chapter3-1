...
import { notificationOptions } from '@/entities/event/config/constants';
...
export const EventListView = () => {
  const { editEvent, events, deleteEvent } = useEventForm();
  const { notifiedEvents } = useNotifications(events);
  const { view, currentDate } = useCalendarView();
  const { filteredEvents, searchTerm, setSearchTerm } = useSearch(events, currentDate, view);
